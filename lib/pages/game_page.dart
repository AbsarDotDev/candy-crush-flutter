...
class _GamePageState extends State<GamePage>
  ...
  void _onTap() async {
    ...
    if (gestureFromTile != null && Tile.isBomb(gestureFromTile!.type!)) {
      ...
      _allowGesture = false;
      // Remove the bomb tile from the grid
      gameBloc.gameController.grid[gestureFromTile!.row][gestureFromTile!.col]
          .visible = false;
      setState(() {});

      // Proceed with explosion
      gameBloc.gameController.proceedWithExplosion(gestureFromTile!, gameBloc);
      // Calculate the position of the tapped tile relative to the screen
      final RenderBox overlay =
          Overlay.of(context)!.context.findRenderObject() as RenderBox;
      final Offset overlayTopLeft = overlay.localToGlobal(Offset.zero);
      final double xPos = gestureFromTile!.x! + overlayTopLeft.dx;
      final double yPos = gestureFromTile!.y! + overlayTopLeft.dy;

      // Overlay a circular progress indicator at that position
      OverlayEntry progressIndicatorOverlay = OverlayEntry(
        builder: (context) {
          return Positioned(
            left: xPos,
            top: yPos,
            child: LottieBuilder.asset(
              'assets/animations/explosion_animation.json',
              repeat: true,
              width: gestureFromTile!.level!.tileWidth,
              height: gestureFromTile!.level!.tileHeight,
            ), // or any other widget you want to show
          );
        },
      );
      Overlay.of(context).insert(progressIndicatorOverlay);

      // Play explosion
      Audio.playAsset(AudioType.bomb);
      await Future.delayed(Duration(seconds: 1), () {
        progressIndicatorOverlay.remove();
      });
